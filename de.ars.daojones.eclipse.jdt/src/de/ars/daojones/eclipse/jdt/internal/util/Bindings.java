...
/*
 * This class was copied and customized from org.eclipse.jdt.ui, Package org.eclipse.jdt.internal.corext.dom.
 */
...
public class Bindings {
	...
	/**
	 * Checks if the declarations of two bindings are equals. Also works across
	 ...
	 * <p>
	 * If the binding resolves to a generic type, the fully qualified name of
	...
	private static void createName(ITypeBinding type, boolean includePackage,
			List list) {
	 ...
	 * @param parameters
	 *            The parameter types of the method to find. If
	 *            <code>null</code> is passed, only the name is matched and
	 *            parameters are ignored.
	 * @return the method binding representing the method
	 */
	public static IMethodBinding findMethodInType(ITypeBinding type,
			String methodName, ITypeBinding[] parameters) {
		if (type.isPrimitive())
			return null;
	...
	public static IMethodBinding findOverriddenMethod(
		...
		if (type.getSuperclass() != null) {
			IMethodBinding res = findOverriddenMethodInHierarchy(type
					.getSuperclass(), overriding);
			if (res != null && !Modifier.isPrivate(res.getModifiers())) {
				if (!testVisibility
						|| isVisibleInHierarchy(res, overriding
		...
		ITypeBinding[] interfaces = type.getInterfaces();
		for (int i = 0; i < interfaces.length; i++) {
			IMethodBinding res = findOverriddenMethodInHierarchy(interfaces[i],
					overriding);
			if (res != null) {
	 ...
	 */
	// TODO: rename to isErasureEquivalentMethod and change to two
	// IMethodBinding parameters
	@Deprecated
	public static boolean isEqualMethod(IMethodBinding method,
			String methodName, ITypeBinding[] parameters) {
		...
		if (methodParameters.length != parameters.length)
			return false;
		for (int i = 0; i < parameters.length; i++) {
			if (!equals(methodParameters[i].getErasure(), parameters[i]
					.getErasure()))
				return false;
		}
		// Can't use this fix, since some clients assume that this method tests
		// erasure equivalence:
		// if (method.getTypeParameters().length == 0) {
		// //a method without type parameters cannot be overridden by one that
		// declares type parameters -> can be exact here
		// for (int i= 0; i < parameters.length; i++) {
		// if ( ! (equals(methodParameters[i], parameters[i])
		// || equals(methodParameters[i].getErasure(), parameters[i]))) //
		// subsignature
		// return false;
		// }
		// } else {
		// //this will find all overridden methods, but may generate false
		// positives in some cases:
		// for (int i= 0; i < parameters.length; i++) {
		// if (!equals(methodParameters[i].getErasure(),
		// parameters[i].getErasure()))
		// return false;
		// }
		// }
		return true;
	...
	public static boolean isSubsignature(IMethodBinding overriding,
		...
		if (m1TypeParams.length != m2TypeParams.length
				&& m1TypeParams.length != 0) // non-generic m1 can override a
												// generic m2
			return false;
	 ...
	 * @param methodName
	 * @param parameters
	 * @return <code>true</code> iff the method m1 (with name
	 *         <code>methodName</code> and method parameters
	 *         <code>parameters</code>) is a subsignature of the method
	 *         <code>m2</code>. Accessibility and return types are not taken
	 *         into account.
	 */
	public static boolean isEqualMethod(IMethodBinding method,
			String methodName, String[] parameters) {
	...
	public static ITypeBinding findTypeInHierarchy(ITypeBinding hierarchyType,
		...
		if (superClass != null) {
			ITypeBinding res = findTypeInHierarchy(superClass,
	...
	public static IVariableBinding getAssignedVariable(Assignment assignment) {
		...
		case ASTNode.QUALIFIED_NAME:
			return (IVariableBinding) ((QualifiedName) leftHand).getName()
					.resolveBinding();
	 ...
	 */
	public static boolean isSuperType(ITypeBinding possibleSuperType,
			ITypeBinding type) {
	...
	/**
	 * Finds the compilation unit where the type of the given
	 * <code>ITypeBinding</code> is defined, using the class path defined by the
	 * given Java project. Returns <code>null</code> if no compilation unit is
	 * found (e.g. type binding is from a binary type)
	 * 
	 * @param typeBinding
	...
	public static ICompilationUnit findCompilationUnit(
			ITypeBinding typeBinding, IJavaProject project)
			throws JavaModelException {
	 ...
	 * @deprecated Use
	 *             {@link #findMethodInHierarchy(ITypeBinding, String, String[])}
	 *             or {@link JavaModelUtil}
	 */
	@Deprecated
	public static IMethod findMethod(IMethodBinding method, IType type)
			throws JavaModelException {
	...
	private static boolean sameParameters(IMethodBinding method,
			IMethod candidate) throws JavaModelException {
	...
	private static boolean sameParameter(ITypeBinding type, String candidate,
			IType scope) throws JavaModelException {
		...
		if ((Signature.getTypeSignatureKind(candidate) == Signature.BASE_TYPE_SIGNATURE) != type
				.isPrimitive()) {
			return false;
		}
		...
		if (type.isPrimitive() || type.isTypeVariable()) {
			...
			if (candidate.charAt(Signature.getArrayCount(candidate)) == Signature.C_RESOLVED) {
				return Signature.toString(candidate).equals(
						Bindings.getFullyQualifiedName(type));
			} else {
				String[][] qualifiedCandidates = scope.resolveType(Signature
						.toString(candidate));
	 ...
	 * that can be used in a declaration signature. Anonymous types are
	 * normalized, to the super class or interface. For null or void bindings
	...
	/**
	 * Normalizes the binding so that it can be used as a type inside a
	 * declaration (e.g. variable declaration, method return type, parameter
	 * type, ...). For null bindings Object is returned.
	 * 
	 * @param binding
	 ...
	 */
	public static ITypeBinding normalizeForDeclarationUse(ITypeBinding binding,
			AST ast) {
	...
	/**
	 * Returns the type binding of the node's type context or null if the node
	 * is an annotation, type parameter or super type declaration of a tope
	 * level type. The result of this method is equal to the result of
	...
	public static String getRawQualifiedName(ITypeBinding binding) {
		...
		if (binding.isArray()) {
			String elementTypeQualifiedName = getRawQualifiedName(binding
					.getElementType());
			if (elementTypeQualifiedName.length() != 0) {
				StringBuffer stringBuffer = new StringBuffer(
						elementTypeQualifiedName);
	 ...
	 * @return returns <code>true></code> if the overriding method overrides a
	 *         candidate
	 * @deprecated Need to review: Use
	 *             {@link #isSubsignature(IMethodBinding, IMethodBinding)} if
	 *             the two bindings are in the same hierarchy (directly
	 *             overrides each other), or
	 *             {@link #findMethodInHierarchy(ITypeBinding, String, ITypeBinding[])}
	 *             else.
	 */
	@Deprecated
	 ...
	 * @param overridable
	 *            the overriding method
	 * @return returns <code>true</code> if the overriding method overrrides the
	 *         overridden
	 * @deprecated Need to review: Use
	 *             {@link #isSubsignature(IMethodBinding, IMethodBinding)} if
	 *             the two bindings are in the same hierarchy (directly
	 ...
	 */
	@Deprecated
	public static boolean areOverriddenMethods(IMethodBinding overridden,
	...
	private static boolean areSubTypeCompatible(IMethodBinding overridden,
			IMethodBinding overridable) {

		if (overridden.getParameterTypes().length != overridable
		...
		if (!overriddenReturn.getErasure().isSubTypeCompatible(
				overridableReturn.getErasure()))
			return false;
		...
		for (int index = 0; index < overriddenTypes.length; index++) {
			final ITypeBinding overridableErasure = overridableTypes[index]
					.getErasure();
			final ITypeBinding overriddenErasure = overriddenTypes[index]
					.getErasure();
			if (!overridableErasure.isSubTypeCompatible(overriddenErasure)
					|| !overridableErasure.getKey().equals(
		...
		for (int index = 0; index < overriddenExceptions.length; index++) {
			...
			for (int offset = 0; offset < overridableExceptions.length; offset++) {
				if (overriddenExceptions[index]
						.isSubTypeCompatible(overridableExceptions[offset]))
					checked = true;
			}
			if (!checked)
				return false;
