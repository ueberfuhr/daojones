...
import de.ars.daojones.connections.Constants;
...
public class ConnectionsFileDescriber implements IContentDescriber {
  ...
  public int describe( InputStream contents, IContentDescription description )
    ...
    // short test
    if ( null != description
        && Constants.FILE_IDENTIFIER_VALUE.equals( description
            .getProperty( Constants.FILE_IDENTIFIER_KEY ) ) )
      return VALID;
    // test for content
    try {
            ...
            if ( "configuration".equals( name ) ) {
              for ( int i = 0; i < attributes.getLength() && !result.get(); i++ ) {
                ...
                if ( attributes.getQName( i ).startsWith( "xmlns" )
                    && "http://www.ars.de/daojones/connections/"
                        .equals( attributes.getValue( 0 ) ) ) {
                  result.set( true );
                }
              }
            }
