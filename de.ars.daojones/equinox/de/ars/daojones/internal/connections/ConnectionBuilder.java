...
import static de.ars.daojones.internal.Activator.PLUGIN_ID;
...
/**
 * This class reads the {@link IExtensionRegistry} for connection elements and
 * registers them in to the {@link ConnectionFactoryManager}.
 * 
 * @author <a href="mailto:ralf.zahn@ars.de">Ralf Zahn</a>
 */
public abstract class ConnectionBuilder {

  private static final String EXTENSION_CREDENTIALS = PLUGIN_ID
      + ".connectioncredentials";
  private static final String EXTENSION_FACTORIES = PLUGIN_ID
      + ".connectionfactories";
  private static final String EXTENSION_CONNECTIONS = PLUGIN_ID
      + ".connections";

  private static ApplicationContext getContext( String id ) {
    return ApplicationContextFactory.getInstance().getApplicationContext(
        null != id ? id : ApplicationContext.DEFAULT_ID );
  }

  private static final Collection<String> applicationContextIds = new HashSet<String>();

  /**
   * Reads the {@link IExtensionRegistry} for connection elements and registers
   * them in to the {@link ConnectionFactoryManager}.
   */
  public static void buildConnectionsFromExtensions() {
    ...
    final IConfigurationElement[] factoryElements = registry
        .getConfigurationElementsFor( EXTENSION_FACTORIES );
    for ( IConfigurationElement factoryElement : factoryElements ) {
      ...
      try {
        ...
        md.setInstance( ( ConnectionFactory ) factoryElement
            .createExecutableExtension( "class" ) );
        ConnectionFactoryManager.getInstance().register( md );
      } catch ( InvalidRegistryObjectException e ) {
        log( IStatus.ERROR, "Connection Factory \"" + md.getName()
    ...
    // read credentials
    final IConfigurationElement[] credentialElements = registry
        .getConfigurationElementsFor( EXTENSION_CREDENTIALS );
    for ( IConfigurationElement credentialElement : credentialElements ) {
      if ( "UserPassword".equals( credentialElement.getAttribute( "type" ) ) ) {
        ...
        log( IStatus.INFO, "Successfully initialized credential for user \""
            + c.getUsername() + "\"." );
      }
    }
    ...
    final IConfigurationElement[] connectionElements = registry
        .getConfigurationElementsFor( EXTENSION_CONNECTIONS );
    for ( IConfigurationElement connectionElement : connectionElements ) {
      if ( "connection".equals( connectionElement.getName() ) ) {
        final ConnectionFactoryMetaData md = ConnectionFactoryManager
            .getInstance().getFactory(
                connectionElement.getAttribute( "factory" ) );
        final Connection con = new Connection() {
          ...
          public ConnectionFactory createConnectionFactory()
            ...
            return null != md && null != md.getInstance() ? md.getInstance()
        ...
        con.setDefault( "true".equals( connectionElement
            .getAttribute( "default" ) ) );
        con.setHost( connectionElement.getAttribute( "host" ) );
        con.setDatabase( connectionElement.getAttribute( "database" ) );
        if ( null != md && null != md.getInstance() )
          con.setFactory( md.getInstance().getClass().getName() );
        con.setCredential( credentials.get( connectionElement
          ...
          if ( "connectionpool".equals( child.getName() ) ) {
            ...
            if ( null != intervalStr ) {
              ...
              if ( intervalStr.endsWith( "w" ) ) {
                con.setCacheExpiration( new Long( intervalStr.substring( 0,
                ...
                con.setCacheExpiration( new Long( intervalStr.substring( 0,
                    intervalStr.length() - 1 ) ) * 60 * 60 * 1000 );
              } else if ( intervalStr.endsWith( "m" ) ) {
                con.setCacheExpiration( new Long( intervalStr.substring( 0,
                    intervalStr.length() - 1 ) ) * 60 * 1000 );
              } else if ( intervalStr.endsWith( "ms" ) ) {
                con.setCacheExpiration( new Long( intervalStr.substring( 0,
                    intervalStr.length() - 2 ) ) );
              } else if ( intervalStr.endsWith( "s" ) ) {
                con.setCacheExpiration( new Long( intervalStr.substring( 0,
                    intervalStr.length() - 1 ) ) * 1000 );
              } else {
            ...
            con
                .setMaxResults( new Integer( child.getAttribute( "maxResults" ) ) );
        ...
        applicationContextIds.add( connectionElement
            .getAttribute( "applicationContext" ) );
      } else if ( "connectionfile".equals( connectionElement.getName() ) ) {
        try {
          final Bundle declaringBundle = Platform.getBundle( connectionElement
              .getContributor().getName() );
          getContext( connectionElement.getAttribute( "applicationContext" ) )
              .createConnections(
                  FileLocator.openStream( declaringBundle, new Path(
                      connectionElement.getAttribute( "filename" ) ), true ) );
          applicationContextIds.add( connectionElement
              .getAttribute( "applicationContext" ) );
          log( IStatus.INFO, "Connections successfully read from file \""
              + connectionElement.getAttribute( "filename" ) + "\"!" );
        } catch ( ConnectionBuildException e ) {
          log( IStatus.ERROR, "Error reading connections from file \""
              + connectionElement.getAttribute( "filename" ) + "\"!", e );
        } catch ( IOException e ) {
          log( IStatus.ERROR, "Error reading connections from file \""
              + connectionElement.getAttribute( "filename" ) + "\"!", e );
        }
