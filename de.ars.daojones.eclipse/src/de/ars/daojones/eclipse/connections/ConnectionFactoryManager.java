...
public class ConnectionFactoryManager {
	...
	public static ConnectionFactoryManager getInstance() {
		if(null == theInstance) theInstance = new ConnectionFactoryManager();
		return theInstance;
	}
	 ...
	 * @param metadata the {@link ConnectionFactoryMetaData}
	 */
	public void register(ConnectionFactoryMetaData metadata) {
		this.factories.put(metadata.getId(), metadata);
		log(IStatus.INFO, "Registered connection factory class for \"" + metadata.getName() + "\".");
	}
	/**
	 * Returns a {@link Map} containing {@link ConnectionFactoryMetaData} objects by their id.
	 * @return the {@link Map} containing {@link ConnectionFactoryMetaData} objects by their id
	 */
